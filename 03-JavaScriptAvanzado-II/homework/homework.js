...
const counterUno = counter();
counterUno();

const counterDos = counter();
counterDos();
...
- cacheFunction debe retornar una función. Esta función debe aceptar un argumento (arg) e invocar a cb con ese argumento; hecho eso, debe guardar el argumento junto con el resultado de la invocación (tip: usá un objeto donde cada propiedad sea el argumento, y su valor el resultado de la correspondiente invocación a cb) de manera que, la próxima vez que reciba el mismo argumento, no sea necesario volver a invocar a cb, porque el resultado estará guardado en la "memoria caché".
  ...
  squareCache(5)    // no volverá a invocar a square, simplemente buscará en la caché cuál es el resultado de square(5) y lo retornará (tip: si usaste un objeto, podes usar hasOwnProperty) */
...
function cacheFunction(cb) {
  ...
  return function (arg) {
    if (obj.hasOwnProperty(arg)) {
      // console.log(`Buscando en memoria este resultado ${arg}`);
      return obj[arg];
    } else {
      const item = cb(arg);
      obj[arg] = item;
      // console.log(`Creando en la memoria cache una propiedad para ${arg}`);
      return item;
    }
  };
}
